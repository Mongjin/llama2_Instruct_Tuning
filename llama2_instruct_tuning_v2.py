...
def format_instruction(sample):
    ...
    return f"""### Instruction: Update 'cur_state' (i.e., current state) based on last user's utterance of [Dialogue]. Follow these rules: First, you should create a dialogue state that helps to request additional information to user for better recommendation. For example, you can expand one or two new state from 'prev_state'. Please diversely create a dialogue state so that bot can request new information to users. Second, update dialogue states of given dialogue. Third, do not generate additional utterances or explain. Please update 'cur_state' while considering these factors. \n ### Input: [Previous state] 'prev_state': {sample['prev_state']} [Dialogue] {dialogue} \n ### Output: [Current state] 'current_state': {sample['cur_state']} """
    # return f"""### Instruction:
